...
- **A folder for every ROM game.** Each game lives in `{platform}/{Game}/`, named after its title and renamed when the title changes, with its manual, extras and mods beside the discs, all offered on its page and with its download. A console sold under two names, such as the Mega Drive and the Genesis, is one platform.
- **A library that keeps up with the disk.** Renaming a file keeps its game, saves and play history. Scans can run on a schedule, follow a list of paths each library should leave alone, and show their progress live with a Stop button. Entries whose files have gone are listed so they can be cleared up.
...
- **Your memory card travels with you.** The card a game writes to is kept on the server as you play and put into whatever browser you sign in from next, so a save made at the desk is there on the laptop. The card already in a browser is never overwritten, and one button takes it out again. When a renamed game turns out to have a card under both names, neither is thrown away: you see both side by side and keep the one you want.
...
- Installed metadata plugins are search sources in their own right: they appear beside the built-in sources in the ROM editor with their own results, and a plugin that supplies artwork is shown above the covers, backgrounds and logos it can provide. Each plugin is asked on its own with a time limit, so one slow or broken source never holds up the rest.
- **Per-platform scrape presets** decide what a ROM scrape downloads: the cover type, the region, and which extra media to fetch, from videos and bezels to wheel logos, Steam Grid banners and the game's manual, which gets a **Manual** button on its page.
...
- **Uploaders own what they bring in.** Games, ROMs and torrents an uploader adds are theirs to remove and count against a quota they can see in their own uploads panel; a torrent that would not fit is refused before it downloads, or as soon as its size is known. Their transfers show in the tray with live seeds, peers, speed and time left, and an administrator can take a game over. An uploader can also add a DLC, an extras pack, a mod or another version to any game they can see, or an extra, a manual or another disc to a ROM game, and take their own files out of it again.
