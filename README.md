...
- **Curate** admins create a collection in one click, set a game's collections in Edit Metadata → Collections, and edit its cover, hero and logo (scrape, upload your own, or revert each to auto - the cover then falls back to a fan of member covers), two descriptions, year range, rating and Time to Beat (auto-average or manual override)
...
- **Single-use password-reset links** the jti of every consumed reset token is added to a Redis blacklist for the rest of its 1 h validity, so the same email link cannot be replayed after a successful reset; the link host is built from a configurable public base URL (Settings > Security) rather than request headers, so it cannot be poisoned by a spoofed Host / X-Forwarded-Host
- **Brute-force protection**Redis-based fixed-window rate limiting per IP with configurable thresholds, ban duration, and whitelist; safe real-IP extraction behind Cloudflare, nginx, or direct access (applied to login, register, and token refresh endpoints); Basic-auth requests pass through the same gate, and forwarded IP headers are only trusted when the connecting peer is a private/LAN address or a configured trusted proxy, so the ban cannot be bypassed by header spoofing
...
- **Network access control** configurable CORS origins, trusted proxy list, and public base URL, dynamic (no restart needed)
- **SSRF guards** server-side URL fetchers resolve the target host and refuse internal addresses: the upload-from-link feature blocks loopback / cloud-metadata / link-local while still allowing a LAN NAS, and plugin store installs are restricted to GitHub or a configured store-source host so an attacker-supplied URL cannot point the installer at an arbitrary server
- **Sanitised HTML** every `v-html` sink (game descriptions, email-template previews, admin dialogs) is run through DOMPurify, and sanitised links that open in a new tab get `rel="noopener noreferrer"`
- **Strong-secret enforcement** the app refuses to start when `GD_AUTH_SECRET_KEY` is a known default or empty value, in every environment, so tokens can never be signed with a forgeable key
